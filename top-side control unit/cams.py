...
class App:
	def __init__(self, window, window_title, video_source1, video_source2, video_source3):
		...
		self.vid1 = MyVideoCapture(self.video_source1, 0.9, 0.9, 1920, 1080)
		self.vid2 = MyVideoCapture(self.video_source2, 0.7, 0.7, 640, 480)
		self.vid3 = MyVideoCapture(self.video_source3, 0.7, 0.7, 640, 480)
		...
		self.canvas1 = tk.Canvas(self.box, width = 1080, height = 600, bg="black",  highlightthickness=0)
		...
		self.canvas1.grid(column=0, row = 0, columnspan = 2)
	...
	def update(self):
		...
		if self.a == 1:
			ret, frame1 = self.vid1.get_frame(0.9, 0.9)
			ret, frame2 = self.vid2.get_frame(0.7, 0.7)
			ret, frame3 = self.vid3.get_frame(0.7, 0.7)
		elif self.s == 1:
			ret, frame1 = self.vid1.get_frame(0.5, 0.5)
			ret, frame2 = self.vid2.get_frame(1.5, 1.5)
			ret, frame3 = self.vid3.get_frame(0.7, 0.7)
		elif self.d == 1:
			ret, frame1 = self.vid1.get_frame(0.5, 0.5)
			ret, frame2 = self.vid2.get_frame(0.7, 0.7)
			ret, frame3 = self.vid3.get_frame(1.5, 1.5)
		...
		if ret:
			...
			if self.a == 1:
				...
				self.canvas2.create_image(200,0, image = self.photo2, anchor=tk.NW)
				self.canvas3.create_image(0,0, image = self.photo3, anchor=tk.NW)
			elif self.s == 1:
				self.canvas1.create_image(550, 250, image = self.photo2, anchor=tk.CENTER)
				self.canvas2.create_image(10,0, image = self.photo1, anchor=tk.NW)
				self.canvas3.create_image(0,0, image = self.photo3, anchor=tk.NW)
			elif self.d == 1:
				self.canvas1.create_image(550, 250, image = self.photo3, anchor=tk.CENTER)
				self.canvas2.create_image(200,0, image = self.photo2, anchor=tk.NW)
