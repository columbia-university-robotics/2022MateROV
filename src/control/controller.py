...
class Controller:
	...
	def __init__(self, ard_port):
		...
		self.motor_arr = [self.MOTOR1, self.MOTOR2, self.MOTOR3, self.MOTOR4, self.MOTOR5, self.MOTOR6]
	...
	def move(self):
		while not self.done:
			...
			self.UD_axis = joystick.get_axis(self.LEFT_VERT)
			self.FB_axis = joystick.get_axis(self.RIGHT_VERT)
			self.rot = joystick.get_axis(self.RIGHT_HORI)
			self.strafe = joystick.get_axis(self.LEFT_HORI)
	...
	def motor_vector(self, UP, FB, rot, strafe):
		...
		if(UP > 0.1 or UP < -0.1):
			UD_val = int(numpy.interp(UP, [-1,1], [1900, 1100]))
			self.MOTOR1 = UD_val
			self.MOTOR2 = UD_val
